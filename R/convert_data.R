...
# file_aa3_txt <- "Data/raw/180430AR1.TXT"
# file_aa3_xlsx <- "Data/raw/180430AR1.xlsx"
# project <- "test"
...
### UPDATE conversion_aa3 2018-10-11

conversion2_aa3 <- function(file_aa3_txt, file_aa3_xlsx, project, topic = NULL, 
                           remote_repos, local_repos, remote, local){
  require(stringr)
  require(readr)
  ...
  if (sum(names(header) != header_control) != 0 |
    ...
    stop("Attention : Erreur durant l'importation ou l'extraction des métadonnées")
  }
  
  # Change NO3 in NOx
  str_replace_all(header$METH, "NO3", "NOx") -> header$METH
  ...
  # Create a several list with the method's information
  method <- list()
  for (i in 2:4) {
  ...
  for (i in which(raw_data$sample_type == "SAMP")) {
    if (is.na(raw_data$project[i]) == TRUE) {
      stop("Attention : Présence de valeurs manquantes dans la colonnes `project`, le fichier xlsx et txt ne correspondent pas entièrement.")
    } 
  }
  attr(raw_data, "method") <- method
